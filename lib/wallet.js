...
let mooncoind = require('bitcoin')

let helpText = require('../text/txt_dash.js').userTxt
let Coin = require('./coin')

const BLOCKCHAIN_EXPLORER_URL = 'https://chainz.cryptoid.info/moon/tx.dws?'
...
module.exports = class Wallet {
  constructor(RPC_PORT, RPC_USER, RPC_PASSWORD, HighBalanceWarningMark, TX_FEE) {
    ...
    this.walletDaemon = new mooncoind.Client({
      host: 'localhost',
      port: RPC_PORT,
      user: RPC_USER,
      pass: RPC_PASSWORD,
      ...
      (resolve, reject) => {
        this.GetBalance(user.id, REQUIRED_TIP_CONFIRMATIONS)
          .then(balance => {
            ...
            balanceText = user.username + helpText.BalanceIs + balance + ' ' + helpText.BaseCurrency
        ...
        this.walletDaemon.getAddressesByAccount(user.id,
          (err, addresses) => {
            ...
            if (addresses !== undefined && addresses.length > 0) {
              ...
              debug('Existing address for ' + user.username + '  ' + depositAddress)
              resolve(user.username + ' you can deposit to: ' + depositAddress)
            } else {
              ...
              this.walletDaemon.getNewAddress(user.id,
                (err, address) => {
                  ...
                  debug('New address for ' + user.username + '  ' + depositAddress)
                  resolve(user.username + ' you can deposit to: ' + depositAddress)
                })
            }
          })
  ...
  Withdraw(value, toAddress, walletPass, user) {
    ...
    return new Promise(
      (resolve, reject) => {
        ...
        if (user.locked === true) {
          let error = user.username + helpText.Locked
          return reject(error)
        }
        ...
        this.GetBalance(user.id, REQUIRED_WITHDRAW_CONFIRMATIONS)
          .then(balance => {
            ...
            if (balance < value) {
              // not enough balance
              user.locked = false
              let error = helpText.InsufficientBalance1 + user.username + helpText.InsufficientBalance2
              return reject(error)
            }
            // enough balance
  ...
  checkBalance(value, userid) {
      this.GetBalance(userid, REQUIRED_TIP_CONFIRMATIONS)
          .then(balance => {
            balance = Coin.toSmall(balance)
            if (balance < value) {
                return reject(false)
          ...
          .catch(err => {
            debug('ERROR checking balance before placing bet: ' + err)
            let error = 'ERROR checking balance before placing bet.'
          ...
          .then(balance => {
            ...
            this.walletDaemon.move(user.id, sendToUser.id, parseFloat(Coin.toLarge(value)),
              (err, result) => {
                ...
                if (result === true) {
                  debug('Sending Tip: Moved ' + Coin.toLarge(value) + ' Mooncoin from ' + user.username + ' to ' + sendToUser.username + '(' + sendToUser.id + ')')
                  // prepare message in channel where Tip command was issued
                  let responses = {
                    public: helpText.SendPublicMessage1 + user.username + helpText.SendPublicMessage2 + sendToUser.username
                  }
                  //  prepare message to recieving user to inform of tip
                  responses.privateToReciever = helpText.SendPrivateMssRecievingUser1 + sendToUser.username +
                    helpText.SendPrivateMssRecievingUser2 + Coin.toLarge(value) + ' ' +
                    helpText.BaseCurrency + helpText.SendPrivateMssRecievingUser3 + user.username + ' !'
                  // prepare message to sending user to inform of new balance
                  this.GetBalanceLine(user)
                    .then(balanceLine => {
                      responses.privateToSender = user.username + helpText.SendPrivateMssSendingUser + balanceLine
                      user.locked = false //  this transaction is done, clear lock to allow new transactions
                  ...
                  // result == false
                  error = helpText.SendOops1 + Coin.toLarge(value) + ' ' + helpText.BaseCurrency + helpText.SendOops2 + sendToUser.username + ''
                  user.locked = false  //  this transaction is done, clear lock to allow new transactions
                  reject(error)
                }
